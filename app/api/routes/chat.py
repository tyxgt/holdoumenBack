...
from app.core.characters import is_valid_character, VALID_CHARACTERS
...
async def chat(
    ...
    """聊天接口。"""
    if not is_valid_character(payload.character):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"无效的角色名: {payload.character}。可用角色: {', '.join(VALID_CHARACTERS)}",
        )
